...
hypoxia_1<-CreateSeuratObject(hypoxia_1, min.cells=3, min.genes=200, project="hypoxia_5x")
hypoxia_2<-CreateSeuratObject(hypoxia_2, min.cells=3, min.genes=200, project="hypoxia_5x")
normoxia_1<-CreateSeuratObject(normoxia_1, min.cells=3, min.genes=200, project="normoxia_5x")
normoxia_2<-CreateSeuratObject(normoxia_2, min.cells=3, min.genes=200, project="normoxia_5x")
```

```{r make_list, echo=TRUE, message=FALSE, warning=FALSE, paged.print=TRUE}
# make list of seurat objects for easier handling
obj<-list(hypoxia_1=hypoxia_1,hypoxia_2=hypoxia_2, normoxia_1=normoxia_1, normoxia_2=normoxia_2, hifko_1=hifko_1, hifko_2=hifko_2)
...
percent.mito<-lapply(obj, function(x) Matrix::colSums(x@raw.data[mito$hypoxia_1, ])/Matrix::colSums(x@raw.data))
...
obj[[1]] <- FilterCells(obj[[1]], subset.names = c("nGene", "percent.mito"), low.thresholds = c(500, -Inf), high.thresholds = c(6000, 0.2))
obj[[2]] <- FilterCells(obj[[2]], subset.names = c("nGene", "percent.mito"), low.thresholds = c(500, -Inf), high.thresholds = c(6000, 0.2))
obj[[3]] <- FilterCells(obj[[3]], subset.names = c("nGene", "percent.mito"), low.thresholds = c(500, -Inf), high.thresholds = c(7500, 0.2))
obj[[4]] <- FilterCells(obj[[4]], subset.names = c("nGene", "percent.mito"), low.thresholds = c(500, -Inf), high.thresholds = c(7500, 0.2))
o
```

# Normalize data
```{r normalize_data, echo=TRUE, message=FALSE, warning=FALSE, paged.print=TRUE}
obj <- lapply(obj, function(x) NormalizeData(x, normalization.method = "LogNormalize", scale.factor = 10000))
```

# Define variable genes
```{r var_genes, echo=TRUE, message=FALSE, warning=FALSE, paged.print=TRUE}
obj<- lapply(obj, function(x) FindVariableGenes(object = x, mean.function = ExpMean, dispersion.function = LogVMR, 
    x.low.cutoff = 0.0125, x.high.cutoff = 3, y.cutoff = 0.5))
```

# Scale data and regress out nUMI and percent.mito
```{r scale_data, echo=TRUE, message=FALSE, warning=FALSE, paged.print=TRUE}
obj<- lapply(obj, function(x) ScaleData(object = x, vars.to.regress = c("nUMI", "percent.mito")))
```
# Save objects
```{r save_obj, echo=TRUE, message=FALSE, warning=FALSE, paged.print=TRUE}
